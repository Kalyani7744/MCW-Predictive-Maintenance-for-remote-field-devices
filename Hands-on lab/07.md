...
4. From the search bar, type _queue_ to filter connectors and triggers. Then, select the **When there are messages in a queue (V2)** item from the filtered list of Triggers.

   ![The trigger dialog is displayed. Create a trigger to respond to a message in the Azure Queue is selected.](media/LAB71.png "Select Queue Trigger")
...
5. Fill out the form as follows, then select the **Create** button:
   ...
   ![The queue connection configuration fields are displayed. The create button is circled.](media/LAB72.png "Create Queue Step")

6. In the queue step, select the Genetrated storage account name which starts with **storageaccount** and Queue name **flownotificationqueue** item, then select the **+ New step** button.

   ![Message queue step event dialog is displayed. The New step button is circled.](media/LAB73.png "Select Queue")
...
10. In the search bar for the next step, search for _queue_ once more, then select the **Delete message (V2)** item from the filtered list of Actions.
    ...
    ![The Flow Action pane is displayed. The queue text is listed in the search field.](media/LAB74.png "Delete queue message step")
